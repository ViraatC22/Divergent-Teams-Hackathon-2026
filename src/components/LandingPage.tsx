  ...
  return (
    <>
      <style>{`
        ...
        .lp-blob-layer { position: absolute; border-radius: 50%; mix-blend-mode: screen; }
        ...
        .lp-blob-fade { position:absolute; bottom:0; left:0; right:0; height:220px; background: linear-gradient(to bottom, transparent, #09090b); pointer-events:none; }
        ...
        .lp-scroll-hint { position:absolute; bottom:36px; left:50%; font-family:'JetBrains Mono',monospace; font-size:9px; letter-spacing:0.18em; color:#3f3f46; animation: scrollBounce 2.4s ease-in-out infinite; }
        ...
        .lp-auth-signin:hover { background: rgba(255,255,255,0.06); border-color: rgba(255,255,255,0.22); color: #e4e4e7; }
        .lp-auth-signup {
          background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.35); color: #22c55e;
        }
        .lp-auth-signup:hover { background: rgba(34,197,94,0.18); border-color: rgba(34,197,94,0.55); }
        .lp-auth-dash {
          background: rgba(34,197,94,0.12); border: 1px solid rgba(34,197,94,0.4); color: #22c55e;
        }
        .lp-auth-dash:hover { background: rgba(34,197,94,0.2); }
      `}</style>
      ...
      <div style={{ minHeight: '100vh', background: '#09090b', color: '#e4e4e7', fontFamily: "'DM Sans', sans-serif", overflowX: 'hidden' }}>
        ...
        <div style={{
          ...
          {isSignedIn ? (
            <>
              <button className="lp-auth-btn lp-auth-dash" onClick={onEnter}>
                DASHBOARD →
              </button>
            ...
            <>
              <button className="lp-auth-btn lp-auth-signin" onClick={() => openSignIn()}>
                Sign In
              </button>
              <button className="lp-auth-btn lp-auth-signup" onClick={() => openSignUp()}>
                Sign Up
              </button>
            </>
          )}
        ...
        <section style={{
          ...
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: 180 }}>
            <h1 style={{
              ...
              fontSize: 'clamp(56px, 11vw, 110px)', letterSpacing: '0.04em',
              color: '#f4f4f5', lineHeight: 0.95, margin: '0 0 14px',
            }}>
              AGRISWARM
            </h1>
            <p className="lp-mono" style={{ fontSize: 11, letterSpacing: '0.22em', color: '#52525b', textTransform: 'uppercase', margin: '0 0 52px' }}>
        ...
        {/* ── TECH STACK ──────────────────────────────────────────────────── */}
        <section ref={setRef(5) as any} style={{ padding: '48px 24px', textAlign: 'center', ...vis(5) }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center', maxWidth: 700, margin: '0 auto' }}>
            {['ESP32', 'BMP280', 'MPU6050', 'FSR402', 'React 18', 'TypeScript', 'WebSocket', 'Vite', 'Express', 'Clerk', 'Supabase', 'Tailwind CSS'].map(tag => (
